...
class ProjectTaskManager:
    ...
    def create_project_tab(self):
        ...
        self.project_id = ttk.Entry(form_frame, width=40)
        self.project_id.grid(row=1, column=1, pady=5, padx=10)
        ...
        # Delete button
        ttk.Button(list_frame, text="Delete Selected Project", 
                  command=self.delete_project).pack(pady=10)
    ...
    def create_task_tab(self):
        ...
        # Subtask
        ttk.Label(form_frame, text="Subtask of:").grid(row=1, column=0, sticky='w', pady=5)
        self.task_parent = ttk.Combobox(form_frame, width=38, state='readonly')
        self.task_parent.grid(row=1, column=1, pady=5, padx=10)
        ...
        self.task_priority.grid(row=2, column=1, pady=5, padx=10)
        self.task_priority.current(2)
        
        # Mandatory
        self.task_mandatory = tk.BooleanVar()
        ttk.Checkbutton(form_frame, text="Mandatory Task", 
                       variable=self.task_mandatory).grid(row=3, column=1, sticky='w', pady=5, padx=10)
        ...
        self.task_start_date.grid(row=4, column=1, pady=5, padx=10)
        
        # End Date
        ttk.Label(form_frame, text="End Date:").grid(row=5, column=0, sticky='w', pady=5)
        self.task_end_date = DateEntry(form_frame, width=37)
        self.task_end_date.grid(row=5, column=1, pady=5, padx=10)
        
        # Time In
        ttk.Label(form_frame, text="Time In (HH:MM):").grid(row=6, column=0, sticky='w', pady=5)
        time_in_frame = ttk.Frame(form_frame)
        time_in_frame.grid(row=6, column=1, pady=5, padx=10, sticky='w')
        ...
        # Time Out
        ttk.Label(form_frame, text="Time Out (HH:MM):").grid(row=7, column=0, sticky='w', pady=5)
        time_out_frame = ttk.Frame(form_frame)
        time_out_frame.grid(row=7, column=1, pady=5, padx=10, sticky='w')
    ...
    def create_project(self):
        ...
        if not name or not pid:
            messagebox.showwarning("Warning", "Please enter project name and ID")
            return
        
        if pid in self.projects:
            messagebox.showwarning("Warning", "Project ID already exists")
            return
    ...
    def on_project_select_task(self, event):
        ...
        task_list = ['(None - Main Task)'] + [f"{tid} - {t['name']}" for tid, t in tasks.items()]
    ...
    def show_progress(self):
        ...
        for tid, task in sorted_tasks:
            if not task.get('parent'):  # Main tasks only
        ...
        for pid, project in self.projects.items():
            ...
            for tid, task in tasks.items():
                ...
                if task_start <= today <= task_end:
                    ...
                    # Insert main task or subtask
                    parent_text = f" (subtask of {task['parent']})" if task.get('parent') else ""
                    
                    item = self.today_tree.insert('', 'end', text=tid, values=(
                        project['name'],
                        task['name'] + parent_text,
                        task['priority'],
                        'Yes' if task['mandatory'] else 'No',
                        task['time_in'],
                        task['time_out'],
    ...
    def mark_today_complete(self):
        """Mark selected task as complete from today's view"""
    ...
    def mark_filter_complete(self):
        """Mark selected task as complete from filter view"""
    ...
    def mark_filter_incomplete(self):
        """Mark selected task as incomplete from filter view"""
        selected = self.filter_tree.selection()
        if not selected:
            messagebox.showwarning("Warning", "Please select a task")
            return
